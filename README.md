...
Version `0.0.2` ships Hermes iOS build `260318099.0.1`. A GitHub Release on that tag holds `hermesvm.xcframework.zip` and a matching macOS `hermesc`. Compiler and VM match. The XCFramework carries Hermes and JSI headers. The library product includes a stub target so Xcode can embed the binary.
